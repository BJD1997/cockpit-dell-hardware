    ...
    const getIpmiInfo = async () => {
      ...
      try {
        const proc = await unwrap(server.execute(
          new Command(["/usr/share/cockpit/dell-system/scripts/ipmi"], { superuser: "require" })
        ));
        ...
        ipmi.value = ipmiInfo;
          ...
          fatalErrorMsg.value.push("An error occurred when trying to run /usr/share/cockpit/dell-system/scripts/ipmi");
          showFixButton.value = false;
        }
    };
