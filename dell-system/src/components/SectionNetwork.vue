    ...
    const getNetworkInfo = async () => {
      ...
      try {
        const proc = await unwrap(server.execute(
          new Command(["/usr/share/cockpit/dell-system/scripts/network"], { superuser: "require" })
        ));
        ...
        networkInfo.forEach(network => {
          ...
          fatalErrorMsg.value.push("An error occurred when trying to run /usr/share/cockpit/dell-system/scripts/network");
          showFixButton.value = false;
        }
    };
