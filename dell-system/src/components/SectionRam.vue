    ...
    const getRamInfo = async () => {
      ...
      try {
        const proc = await unwrap(server.execute(
          new Command(["/usr/share/cockpit/dell-system/scripts/ram"], { superuser: "require" })
        ));
        ...
        ramInfo.forEach(ram => {
          ...
          fatalErrorMsg.value.push("An error occurred when trying to run /usr/share/cockpit/dell-system/scripts/ram");
          showFixButton.value = false;
        }
    };
