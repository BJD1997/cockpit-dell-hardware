    ...
    const getCpuInfo = async () => {
      ...
      try {
        const proc = await unwrap(server.execute(
          new Command(["/usr/share/cockpit/dell-system/scripts/cpu_info"], { superuser: "require" })
        ));
        ...
        cpuInfo.cpus.forEach(cpu => {
          ...
          fatalErrorMsg.value.push("An error occurred when trying to run /usr/share/cockpit/dell-system/scripts/cpu_info");
          showFixButton.value = false;
        }
    };
