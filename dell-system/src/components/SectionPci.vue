    ...
    const getPciInfo = async () => {
      ...
      try {
        const proc = await unwrap(server.execute(
          new Command(["/usr/share/cockpit/dell-system/scripts/pci"], { superuser: "try" })
        ));
        const pciInfo = JSON.parse(proc.getStdout());
        pcis.value.length = 0;
        pciInfo.forEach(pci => {
          pcis.value.push(pci);
        });
        ...
        fatalErrorMsg.value.push("An error occurred when trying to run /usr/share/cockpit/dell-system/scripts/pci");
      }
    };
